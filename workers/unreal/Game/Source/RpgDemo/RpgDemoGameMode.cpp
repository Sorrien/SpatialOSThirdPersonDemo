	...
	const FVector& position)
{
	...
	const Coordinates initialPosition{ spatialOsPosition.X, spatialOsPosition.Y,
									  spatialOsPosition.Z };
	const improbable::transform::Rotation initialRotation{ 0,0,0 };
	const improbable::Vector3f initialLocation{ spatialOsPosition.X, spatialOsPosition.Y, spatialOsPosition.Z };
	const improbable::Vector3f initialVelocity{ 0.0f, 0.0f, 0.0f };
	...
	auto playerTemplate =
		...
		.AddMetadataComponent(Metadata::Data{ "PlayerCharacter" })
		.SetPersistence(false)
		.SetReadAcl(globalRequirementSet)
		.AddComponent<player::HeartbeatSender>(player::HeartbeatSender::Data{},
			workerRequirementSet)
		.AddComponent<improbable::transform::Transform>(improbable::transform::Transform::Data{ initialLocation, initialRotation, initialVelocity },
			workerRequirementSet)
		.AddComponent<player::HeartbeatReceiver>(player::HeartbeatReceiver::Data{},
			workerRequirementSet)
		.AddComponent<player::MovementInputSender>(player::MovementInputSender::Data{},
			ownClientRequirementSet)
		.AddComponent<player::MovementInputReceiver>(player::MovementInputReceiver::Data{},
			workerRequirementSet)
		.Build();
